			...
			for (QStringList::Iterator i = list.begin(); i!= list.end(); i++)
			{
				...
				if (info->exists())
				{
					...
					else
					{
						...
						if (GetFileInfo(info, fsInfo))
						{
							PRINT("GETFILEINFO: Success\n");
							MessageRef ref(new Message(), NULL);
							// <postmaster@raasu.org> 20021023 -- Use temporary variable so that you save unnecessary function call repeating
							QCString qcPath = info->dirPath(true).utf8();
							ref()->AddInt32("beshare:Modification Time", fsInfo->fModificationTime);
							ref()->AddString("beshare:Kind", (const char *) fsInfo->fMIME.utf8());	// give BeSharer's some relief
							ref()->AddString("beshare:Path", (const char *) qcPath);
							ref()->AddString("winshare:Path", (const char *) qcPath);	// secret path
							ref()->AddInt64("beshare:File Size", info->size());
							ref()->AddString("beshare:FromSession", (const char *) fNet->LocalSessionID().utf8());
							ref()->AddString("beshare:File Name", (const char *) info->fileName().utf8());
							
							QString nodePath = rpath;
							nodePath += info->fileName();
							...
							PRINT("GETFILEINFO: FAILED ON %s\n", (const char *) info->fileName().utf8());
