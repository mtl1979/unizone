...
public:
	...
	bool IsConnected() const { return wmt->IsInternalThreadRunning(); }
	...
	{ 
		return wmt->SendMessageToSessions(msgRef, optDistPath); 
	}
	
	bool IsInternalThreadRunning() const
	{
		return wmt->IsInternalThreadRunning();
	}
	
	status_t SetOutgoingMessageEncoding(int32 encoding, const char * optDistPath = NULL)
	{
		return wmt->SetOutgoingMessageEncoding(encoding, optDistPath);
	}
	...
	{
		return wmt->GetNextEventFromInternalThread(retEventCode, optRefMsgRef, optFromSession, optFromFactory);
	}
