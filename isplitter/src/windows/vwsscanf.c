...
int __cdecl _filwbuf (FILE *str);

#define _getwc_lk(_stream)       (--(_stream)->_cnt >= 0 ? *((wchar_t *)(_stream->_ptr))++ : _filwbuf(_stream))                         /* _MTHREAD_ONLY */
...
/* Neither coerceshort nor farone are need for the 386 */
    ...
    */
    ...
    while (*format) {
        ...
        if (iswspace((wchar_t)*format)) {

            UN_INC(EAT_WHITE()); /* put first non-space char back */

            while ((iswspace)(*++format)); /* NULL */
        ...
        if (L'%' == *format) {
            ...
            while (!done_flag) {
                ...
                comchr = *++format;
                if (_ISDIGIT((wchar_t)comchr)) {
                    ++widthset;
                    width = MUL10(width) + (comchr - L'0');
                } else
                    switch (comchr) {
                        case L'F' :
                        case L'N' :   /* no way to push NEAR in large model */
...
DEFAULT_LABEL:
            ...
            comchr = *format | (L'a' - L'A');

            if (L'n' != comchr)
                if (L'c' != comchr && LEFT_BRACKET != comchr)
                    ch = EAT_WHITE();
                else
                    ch = INC();
...
scanit:
                    ...
                    case L'i' :      /* could be d, o, or x */

                        comchr = L'd'; /* use as default */
...
x_incwidth:
                        ...
                        if (L'0' == ch) {
                            ...
                            if (L'x' == (wchar_t)(ch = INC()) || L'X' == (wchar_t)ch) {
                                ch = INC();
                                if (widthset) {
                                    width -= 2;
                                    if (width < 1)
...
#endif  /* _INTEGRAL_MAX_BITS >= 64    */
                        if (L'F'==comchr) /* expected ':' in long pointer */
                            started = 0;
...
assign_num:
                    ...
                    case L'n' :      /* char count, don't inc return value */
                        number = charcount;
                        if(!suppress)
                        ...
                        if (started)
                            if (!suppress) {
                                ...
                                *wptr = '\0';
                                ...
                                if (longone-1)
									*(double*)pointer = wcstod(floatstring, NULL);
								else
									*(float*)pointer = (float)wcstod(floatstring, NULL);
